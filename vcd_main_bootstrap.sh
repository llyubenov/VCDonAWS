...
function vcd_main_on_cent_os () {
    ...
    export xFerIP=`aws ec2 describe-instances --region=$Region --filters "Name=availability-zone,Values=us-west-2a" "Name=tag:Name,Values='vCD Transfer Server'" --query 'Reservations[*].Instances[*].[PrivateIpAddress]' | sed -n '4p' | sed -e 's/ //g' -e 's/^"//' -e 's/"$//'`
...
at <<'EOF' >> ~/cloudwatchlog.conf

[console-proxy.log.log]
datetime_format = %b %d %H:%M:%S
file = /opt/vmware/vcloud-director/logs/console-proxy.log.log
buffer_duration = 5000
log_stream_name = {hostname}
initial_position = start_of_file
EOF
    cat /tmp/console-proxy.log.txt >> ~/cloudwatchlog.conf
